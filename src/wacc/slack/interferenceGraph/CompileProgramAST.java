...
public class CompileProgramAST {
	...
	public String compile() {		
		...
		try {
			...
			GenerateOptimizedIntermediateCode generateOptimizedIntermediateCode = new GenerateOptimizedIntermediateCode(program.getStatements(),optimisationLevel);
			Deque<PseudoInstruction> body = generateOptimizedIntermediateCode.call();
			
			
			Operand stackSpace = new ImmediateValue(generateOptimizedIntermediateCode.getNumberOfRegsUsed());
	...
	private void initDataSection() {
		dataSection.add(new Label(IntermediateCodeGenerator.INT_SCANF_STORE_LABEL));
	...
	private void initTextSection() {
		...
		textSection.add(new Label(IntermediateCodeGenerator.STRING_FORMAT_LABEL));
		...
		textSection.add(new Label(IntermediateCodeGenerator.TRUE_LABEL.getName()));
		textSection.add(new AssemblerDirective("\t.ascii \"true\\0\""));

		textSection.add(new Label(IntermediateCodeGenerator.FALSE_LABEL.getName()));
	...
	private Deque<PseudoInstruction> divideByZeroError() throws Exception {
		...
		//instrList.addAll(IntermediateCodeGenerator.DIVIDE_BY_ZERO_ERROR.accept(new IntermediateCodeGenerator()));
		instrList.addAll(new GenerateOptimizedIntermediateCode(IntermediateCodeGenerator.DIVIDE_BY_ZERO_ERROR,0).call());
	...
	private Deque<PseudoInstruction> nullReferenceErrorAsm() throws Exception {
		...
		//instrList.addAll(IntermediateCodeGenerator.NULL_REFERENCE_ERROR.accept(new IntermediateCodeGenerator()));
		instrList.addAll(new GenerateOptimizedIntermediateCode(IntermediateCodeGenerator.NULL_REFERENCE_ERROR,0).call());
	...
	private Deque<PseudoInstruction> negativeIndexExceptionAsm() throws Exception{
		Deque<PseudoInstruction> instrList = new LinkedList<PseudoInstruction>();
		instrList.add(new Label("p_negative_index_exception"));
	//	instrList.addAll(IntermediateCodeGenerator.NEGATIVE_INDEX_ERROR.accept(new IntermediateCodeGenerator()));
		instrList.addAll(new GenerateOptimizedIntermediateCode(IntermediateCodeGenerator.NEGATIVE_INDEX_ERROR,0).call());
	...
	private Deque<PseudoInstruction> largeIndexExceptionAsm() throws Exception {
		...
		//instrList.addAll(IntermediateCodeGenerator.LARGE_INDEX_ERROR.accept(new IntermediateCodeGenerator()));
		instrList.addAll(new GenerateOptimizedIntermediateCode(IntermediateCodeGenerator.LARGE_INDEX_ERROR,0).call());
		// Exit the program
		instrList.add(new Mov(ArmRegister.r0, new ImmediateValue(-1)));
		instrList.add(new BLInstruction("exit"));
