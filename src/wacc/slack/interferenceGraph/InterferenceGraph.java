...
public class InterferenceGraph extends AbstractGraph<InterferenceGraphNode> {
	...
	private InterferenceGraph(Map<CFGNode, Set<Register>> liveOut,
			Set<Register> allRegisters) {
		...
		for (Register t : allRegisters) {
			...
			for (CFGNode id : liveOut.keySet()) {
				...
				if (liveOutId.contains(t)) {
					...
					this.putNode(InterferenceGraphNode
							.getInterferenceGraphNodeForRegister(t),
							convertRegisters(liveOutId, t));
				} else {
					// this will only occur when a register does not occur in
					// the liveOut of any node, this deals with "meaningless"
					// statements at the end of the main function
					this.putNode(InterferenceGraphNode
							.getInterferenceGraphNodeForRegister(t),
							new HashSet<InterferenceGraphNode>());
				}
			}
		}
...
//		System.out.println(this);
	...
	private Set<InterferenceGraphNode> convertRegisters(Set<Register> regs,
		...
		for (Register r : regs) {
			...
			if (r != t) {
				n.add(InterferenceGraphNode
						.getInterferenceGraphNodeForRegister(r));
			}
		}
		return n;
	}
